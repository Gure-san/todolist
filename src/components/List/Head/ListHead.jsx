...
export default function ListHead({appData, dispatch, derivedItems}) {
  ...
  let styleHeadSection,
  srcSubmitButton;

  if( theme === THEME_VARIANTS.DARK_MODE ) {
    styleHeadSection = {
      title : "dark:text-tertiary-100",
      inputForm : "dark:bg-secondary-100 dark:text-tertiary-100 dark:selection:bg-primary dark:border-secondary-150 dark:shadow-primary dark:shadow-[inset_0px_0px_1rem_bg-primary]",

      submitButton : "dark:hover:bg-secondary-150 dark:bg-secondary-100 dark:border-secondary-150 dark:shadow-primary dark:shadow-[inset_0px_0px_1rem_bg-primary]" 
    };
  ...
  if( theme === THEME_VARIANTS.LIGHT_MODE ) {
    styleHeadSection = {
      title : "text-primary",
      inputForm : "bg-tertiary-100 text-primary selection:bg-primary selection:text-tertiary-100 border-tertiary-150 shadow-tertiary-150 shadow-[inset_0px_0px_1rem_bg-tertiary-150]",

      submitButton : "hover:bg-tertiary-150 bg-tertiary-100 border-tertiary-150 shadow-tertiary-150 shadow-[inset_0px_0px_1rem_bg-tertiary-150]" 
    };
  ...
  return (
    <section className="flex justify-between items-center flex-wrap w-full pr-1">
      {/* Section Name */}
      <h1 className={`${styleHeadSection.title} font-bold text-xl tracking-wider`}>
        Daftar Kategori
      </h1>
      ...
      onSubmit={(e) => {
        e.preventDefault();
        dispatch({
          type : editModeState && editModeState.editMode 
          ? HANDLE_CASE.EDIT
          : HANDLE_CASE.ADD,

          payload: editModeState && editModeState.editMode 
          ? {
            listData,
            value: listName,
            changeListData: true,
